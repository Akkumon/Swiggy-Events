...
const EnhancedEventCard = ({ event, onViewDetails }: EnhancedEventCardProps) => {
  ...
  return (
    <Card 
      className={`overflow-hidden transition-all duration-300 cursor-pointer ${theme.visualElements.shadowStyle} ${isHovered ? 'shadow-lg' : ''}`}
    ...
    >
      ...
      <div className="relative h-40 sm:h-48 md:h-56 bg-gray-200 overflow-hidden">
        ...
        {event.restaurantName && (
          <div className="absolute top-2 left-2 bg-white/95 backdrop-blur-sm rounded-lg px-2 py-1 border border-white/20">
            <div className="flex items-center gap-1">
              <div className="w-6 h-6 bg-orange-500 rounded-full flex items-center justify-center">
                <span className="text-white text-xs font-bold">R</span>
              </div>
              <span className="text-xs font-medium text-gray-900">{event.restaurantName}</span>
            </div>
          </div>
        )}
